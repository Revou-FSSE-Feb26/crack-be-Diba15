...
export class ReportService {
  ...
  async findAll(status?: ReportStatus) {
    const reports = await this.reportRepository.findAllReports(status);
  ...
  async resolve(id: string, curatorId: string, dto: ResolveReportDto) {
    const existing = await this.reportRepository.findReportById(id);
    ...
    const updated = await this.reportRepository.resolveReport(id, curatorId, dto.status);
